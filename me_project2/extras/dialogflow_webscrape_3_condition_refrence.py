...
async def handle_request(request: Request):
    ...
    if intent == "list_bank_execute":
        ...
        if existing_data:
            if existing_data.get('BANK') == "" or existing_data.get('BRANCH_TIMINGS') == "":
                print("Condition Full Web Scrape")
                condition = "fresh_scrape"
                return web_scrapes_next(scrape_url, cleaned_ifsc_codes1, condition)

            # elif all(field =="" for field in ['MICR','BANK_EMAIL', 'BRANCH_TIMINGS', 'CONTACT', 'OFFICIAL_WEBSITE', 'TOLL','BRANCH_CODE', 'MICR', 'PINCODE','TIMESTAMP','FAX']):
            #     print("Condition Partial Web Scrape")
            #     condition = "partial_scrape"
...
def web_scrapes_next(scrape_url, cleaned_ifsc_code1, condition):
    ...
    if condition == "fresh_scrape":
    ...
    elif condition == "partial_scrape":
        print("You are at partial scrape!!!")
        # webscrape dict
        join_data = {
            "BRANCH_TIMINGS": branch_timings,  
            "CONTACT": contact_number,
        ...
        existing_data1 = collection.find_one({'IFSC':cleaned_ifsc_code1})
        ...
        if formatted_data3 == "":
            return JSONResponse(content={
                "fulfillmentText": f"Sorry!!!, IFSC Code: {cleaned_ifsc_code1} does not exist!"
            })
